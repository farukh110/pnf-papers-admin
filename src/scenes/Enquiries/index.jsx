...
const Enquiries = () => {
    ...
    const loadLazyData = useCallback(debounce(() => {
        ...
        dispatch(getAllEnquiries(params))
            .unwrap()
            .catch((error) => {
    ...
    useEffect(() => {
        if (Array.isArray(enquiries)) {
            const enquiryWithSerialNumbers = enquiries.map((enquiry, index) => ({
                ...enquiry,
                serialNumber: lazyState.first + index + 1,
            }));

            setDataSource(enquiryWithSerialNumbers);
        }
    }, [enquiries, lazyState.first]);
    ...
    const actionItems = useMemo(() => [
        {
            ...
            on_action: async () => {
                try {
                    ...
                    const result = await dispatch(getAllEnquiries(allParams)).unwrap();
                    ...
                    exportToExcel(exportData, 'Enquiries_List');
                } catch (err) {
    ...
    return (
        <div className='row'>
            <div className='col-md-12'>
                <h4 className='mt-md-2'>Enquiries List</h4>
